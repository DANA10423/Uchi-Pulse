...
家族対象のActionでは `target_family_id` を必要とする。共通対象のActionでは対象家族を持たない。DB上はNULLを許容し、Action種別のルールによって必須／不要を検証する。
...
例として「太郎の入室NG」というActionを父・母へ通知する設定を可能とする。
...
Actionの詳細実装方式、Action IDの具体的な採番、通知サービス固有仕様は詳細設計で定義する。